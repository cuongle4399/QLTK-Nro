...
public class MainMod : IActionListener, IChatable
{
	...
	public static void Update()
	{
		...
		{
			if (GameCanvas.gameTick % 200 == 0 && !isSlovingCapcha)
			{
	...
	public static void Paint(mGraphics g)
	{
		...
		if (ModProCL.banDo)
		{
			GraphicsManagement.DrawFont.drawString(g, "Đang auto bán đồ khi full", 10, num2, 0);
			num2 += num;
		}
		if (ModProCL.catDoVIP)
		{
			GraphicsManagement.DrawFont.drawString(g, "Đang auto cất đồ vào rương khi full", 10, num2, 0);
			num2 += num;
		}
		if (MainXmapCL.isXmaping)
		{
	...
	public static void SolveCapcha()
	{
		...
		try
		{
			...
			string path = Path.Combine("Data", "keyAPI.ini");
			if (!File.Exists(path))
			{
			...
			string address = "http://api.phamgiang.net/captcha/nro?token=" + text;
			...
			Match match = Regex.Match(text2, "\"captcha\"\\s*:\\s*\"(\\d+)\"");
			Match match2 = Regex.Match(text2, "\"status\"\\s*:\\s*(\\d+)");
			if (match.Success && match2.Success && match2.Groups[1].Value == "0")
			{
				string value2 = match.Groups[1].Value;
				if (value2.Length >= 4 && value2.Length <= 7)
				{
					statusCapcha = "✅ Nhập Captcha: " + value2;
					string text3 = value2;
					foreach (char ch in text3)
					{
				...
				{
					statusCapcha = "⚠ Captcha không hợp lệ: " + value2;
				}
			}
			else
			{
				statusCapcha = "❌ API lỗi hoặc Captcha sai: " + text2;
			}
		}
		catch (WebException ex)
		{
			string text4 = new StreamReader(ex.Response.GetResponseStream()).ReadToEnd();
			statusCapcha = "❌ Lỗi mạng/API: " + text4;
		}
		catch (Exception ex2)
		{
			statusCapcha = "❌ Lỗi: " + ex2.Message;
		}
		finally
		{
