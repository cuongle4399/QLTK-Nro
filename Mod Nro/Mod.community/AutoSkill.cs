...
public class AutoSkill : IActionListener, IChatable
{
    ...
    /// <summary>
    /// TỐI ƯU: Hàm AutoUseSkill cải tiến
    /// - Giảm delay global từ 150ms → 50ms
    /// - Bỏ điều kiện % 20 quá chặt
    /// - Gộp logic trùng lặp thành 1
    /// - Dùng skill nhanh hơn ~3 lần
    /// </summary>
    private static void AutoUseSkill(int skillIndex)
    {
        ...
        // === KIỂM TRA SKILL CÓ TỒN TẠI & CÓ ĐƯỢC DÙNG KHÔNG ===
        ...
        // === NẾU COOLDOWN == 0, SET DELAY MẶC ĐỊNH ===
        ...
        // === KIỂM TRA MANA ĐỦ & KHÔNG ĐANG CHARGE ===
        if (!isMeHasEnoughMP(GameScr.keySkill[skillIndex]) || GameScr.gI().isCharging())
        {
            return;
        }

        // === TỐI ƯU: GIẢM DELAY GLOBAL TỪ 150ms → 50ms ===
        long currentTime = mSystem.currentTimeMillis();
        if (GameCanvas.gameTick % 10 != 0) // Cải tiến: 50ms thay vì 150ms
        {
            return;
        }

        // === LẤY THỜI GIAN KỂ TỪ LẦN DÙNG TRƯỚC ===
        long timeSinceLastUse = currentTime - lastTimeUseSkill[skillIndex];
        long delay = timeAutoSkills[skillIndex];

        // === TỐI ƯU: GỘP LOGIC THÀNH 1 ===
        // Nếu delay == -1: dùng ngay (không delay)
        // Nếu delay > 0: dùng theo thời gian delay
