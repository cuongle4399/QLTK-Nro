...
public class YardatCL : IActionListener
{
    ...
    public void perform(int idAction, object p)
    {
        ...
        {
            Char c = (Char)p;
            listTargetAutoChangeFocus.Add(c);
            GameScr.info1.addInfo("Đã Thêm " + c.cName + " [" + c.charID + "]");
            ShowConfigMenuDanhChuyen(); // mở lại menu Config
            return;
        }
        ...
        switch (idAction)
        {
            case 1:
                ...
                if (isAutoChangeFocus) AutoSkill.isAutoSendAttack = false;
                break;
    ...
    public static void ShowMenu()
    {
        ...
        // nút Config để vào danh sách thêm/xóa
    ...
    public static void ShowConfigMenuDanhChuyen()
    {
        ...
        for (int i = 0; i < GameScr.vCharInMap.size(); i++)
        {
            ...
            if (c == null || c.charID == Char.myCharz().charID || c.isPet || c.isMiniPet || c.charID > 0 || c.cName == null) continue;
    ...
    {
        if (!isAutoChangeFocus || Char.myCharz().isCharge) return;
