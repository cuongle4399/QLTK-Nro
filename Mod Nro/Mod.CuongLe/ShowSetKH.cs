...
public class ShowSetKH
{
    ...
    private static void DemItems(Item[] items, int[] set1, int[] set2, int[] set3, int[] set4, int[] set5)
    {
        ...
        Item[] array12 = array11;
        Item[] array13 = array12;
        foreach (Item item3 in array13)
        {
            try
            {
        ...
        switch (sellState)
        {
            ...
            case 5:
                if (bagIndex >= 0)
                {
                    ...
                    if (item != null && !itemKH(item) && !itemStar(item) && !itemTL(item) && item.template.id < 300 && item.template.type >= 0 && item.template.type <= 4)
                    {
                    ...
                    {
                        bagIndex--;
                        sellTimer = mSystem.currentTimeMillis() + 200;
                        sellState = 5;
                    }
                }
                else
            ...
            case 6:
                if (mSystem.currentTimeMillis() >= sellTimer)
                {
                    ...
                    // Kiểm tra xem hành trang còn FULL không
                    if (ModProCL.isFULLBag())
                    {
                        sellState = 5;
                    }
                    ...
                    {
                        // Hành trang không full nữa, kết thúc bán
                        sellState = 7;
                    }
                }
                break;
    ...
    public static void UpdateCatDo()
    {
        ...
        {
            case 0:
                boxSlotBefore = ModProCL.countEmptyBox();
                if (boxSlotBefore == 0)
                {
                    GameScr.info1.addInfo("|2| Rương FULL rồi");
                    ModProCL.catDoVIP = false;
                }
                else if (ModProCL.isFULLBag() && !ModProCL.isFULLBox())
                {
            ...
            case 4:
                if (catDoIndex >= 0)
                {
                    ...
                    {
                        catDoIndex--;
                        catDoTimer = mSystem.currentTimeMillis() + 200;
                    }
                }
                else
                {
            ...
            case 5:
                ...
                break;
            case 6:
                if (boxSlotBefore == ModProCL.countEmptyBox())
                {
                    GameScr.info1.addInfo("|2| Hành trang không có đồ đủ tiêu chuẩn để cất");
                }
                if (catDoFlagGoBack)
                {
                    AutoTrainCL.isGoBack = true;
