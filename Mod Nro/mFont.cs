...
public class mFont
{
	...
	public void drawString(mGraphics g, string st, int x, int y, int align)
	{
		if (mGraphics.zoomLevel == 1)
		{
			...
			{
				int num13 = strFont.IndexOf(st[i] + string.Empty);
				if (num13 == -1)
				{
					num13 = 0;
				}
				if (num13 > -1)
				{
					int x2 = fImages[num13][0];
					int num14 = fImages[num13][1];
					int w = fImages[num13][2];
					int num15 = fImages[num13][3];
					if (num14 + num15 > imgFont.texture.height)
					{
						num14 -= imgFont.texture.height;
						x2 = imgFont.texture.width / 2;
					}
					g.drawRegion(imgFont, x2, num14, w, num15, 0, num12, y, 20);
				}
				num12 += fImages[num13][2] + space;
			}
		}
		else
		{
	...
	public void drawString(mGraphics g, string st, int x, int y, int align, mFont font)
	{
		if (mGraphics.zoomLevel == 1)
		{
			...
			for (int i = 0; i < length; i++)
			{
				int num13 = strFont.IndexOf(st[i]);
				if (num13 == -1)
				{
					num13 = 0;
				}
				if (num13 > -1)
				{
					...
					{
						g.drawRegion(font.imgFont, x2, num14, w, num15, 0, num12 + 1, y, 20);
						g.drawRegion(font.imgFont, x2, num14, w, num15, 0, num12, y + 1, 20);
					}
					g.drawRegion(imgFont, x2, num14, w, num15, 0, num12, y, 20);
				}
				num12 += fImages[num13][2] + space;
		...
		if (mGraphics.zoomLevel == 1)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				if (num13 > -1)
				{
					int x2 = fImages[num13][0];
					int num14 = fImages[num13][1];
					int num15 = fImages[num13][2];
					int num16 = fImages[num13][3];
					if (num14 + num16 > imgFont.texture.height)
					{
						num14 -= imgFont.texture.height;
						x2 = imgFont.texture.width / 2;
					}
					int w = (int)((float)num15 * 0.85f);
					int h = (int)((float)num16 * 0.85f);
					g.drawRegion(imgFont, x2, num14, w, h, 0, num12, y, 20);
					num12 += (int)((float)fImages[num13][2] * 0.85f) + space;
				}
			}
		}
		else
