...
public static class MobTrainTab
{
	...
	public static void HandleClick(int panelX, int contentY)
	{
		...
		{
			if (flag && orCreateSelectedMobIds.Count > 0)
			{
	...
	private static void ShowSelectByTypeMenu()
	{
		...
		{
			MobTrain mobTrain = list[i];
			int num = 0;
			foreach (MobTrain mobTrain3 in MobTrains)
			{
				if (mobTrain3.TemplateId == mobTrain.TemplateId)
				{
					num++;
				}
			}
			string caption = $"{mobTrain.Name}\n[{NinjaUtil.getMoneys(mobTrain.HP)}HP] - SL:{num}";
			myVector.addElement(new Command(caption, AutoTrainCL.getInstance(), 1, mobTrain.TemplateId));
		}
