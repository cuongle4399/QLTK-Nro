...
public static class MainMenu
{
	...
	private static void UpdateCachedValues()
	{
		...
		int totalItems = num3;
		MenuHelper.UpdateCachedValues(currentTab, totalItems, buttonPanelHeight);
		int num4 = MenuHelper.CalculateMaxScrollOffset(totalItems);
		switch (currentTab)
		{
		case 0:
			if (AutoItemTab.ScrollOffset > num4)
			{
				AutoItemTab.ScrollOffset = num4;
			}
			break;
		case 1:
			if (SkillTrainTab.ScrollOffset > num4)
			{
				SkillTrainTab.ScrollOffset = num4;
			}
			break;
		case 2:
			if (MobTrainTab.ScrollOffset > num4)
			{
				MobTrainTab.ScrollOffset = num4;
			}
			break;
		}
	}
