...
public class MainXmapCL : IActionListener, IChatable
{
    ...
    public static void Update()
    {
        ...
        if (currentMap == IdMapEnd)
        {
            FinishXmap();
            return;
        }

        if (TryEatChicken() || !ShouldUpdateXmap(now) || GameCanvas.isWait())
            return;
    ...
    private static bool ShouldUpdateXmap(float now)
    {
        ...
        if (now - lastWaitTime <= 0.3f) return false;
    ...
    public static void UpdateXmap(int mapID)
    {
        ...
        if (!DataXmap.linkMaps.ContainsKey(999))
            DataXmap.linkMaps[999] = new List<NextMap>();

        var list = DataXmap.linkMaps[999];
        list.Clear();
        ...
        int[] path = XmapPathfinder.GetInstance().FindPath(
            mapID,
            TileMap.mapID,
            me.cPower,
        ...
        if (!TryUseCapsule(path) &&
            !CheckClanRequirement(path) &&
            (!isUsingItem || now - lastItemUseTime >= 0.5f))
        {
