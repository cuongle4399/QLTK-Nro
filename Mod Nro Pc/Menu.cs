public class Menu
{
    ...
    public string menuHeaderText; // Text lớn trên đầu menu
    ...
    public void startAt(MyVector menuItems, int pos)
    {
        ...
        close = false;
        tDelay = 0;
        menuHeaderText = ""; // Xóa header text cũ
        if (menuItems.size() == 1)
        {
            menuSelectedItem = 0;
    ...
    // Hàm đặt text header lớn với avatar

    // Hàm đặt text header lớn (không avatar)
    public void setMenuHeaderText(string text)
    {
        this.menuHeaderText = text;
    }
    ...
    private void drawMenuHeader(mGraphics g)
    {
        ...
        int padding = 6 * zoom;
        int avatarSize = 16*zoom;
        int lineHeight = mFont.tahoma_7b_dark.getHeight() + 2;

        string[] lines = menuHeaderText.Split('\n');

        // Chiều cao hộp
        int boxHeight = padding * 2 + lines.Length * lineHeight;

        // Bám theo menu
        int w = menuItems.size() * menuW;
        int boxWidth = w > GameCanvas.w -30 ? GameCanvas.w - 10: w ;
        int boxX = menuX;

        // Hộp sát menu
        int gapToMenu = 2 * zoom;
        int boxY = menuY - boxHeight - gapToMenu;
        if (boxY < padding)
            boxY = padding;
        ...
        // ===== AVATAR (TRONG HỘP – SÁT VIỀN TRÊN, GÓC TRÁI) =====
     
        // ===== TEXT CĂN GIỮA =====
        int centerX = boxX + boxWidth / 2;
        int textY = boxY + (boxHeight - lines.Length * lineHeight) / 2;
